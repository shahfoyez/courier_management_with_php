...
<?php
	class User{
		...
		public function adminProfileEdit($data, $file, $adminUsername){
			date_default_timezone_set("Asia/Dhaka");
			$updatedAt= date("Y-m-d H:i:s");
			//Table and Billing Data
			$name =$this->fm->validation($data['name']);
			...
			$cnpassword =$this->fm->validation($data['cnpassword']);
			$cnpassword = mysqli_real_escape_string($this->db->link, $cnpassword);   
			$num =$this->fm->validation($data['num']);
			$num = mysqli_real_escape_string($this->db->link, $num);
			$role =$this->fm->validation($data['role']);
			$role = mysqli_real_escape_string($this->db->link, $role);
			$pin =$this->fm->validation($data['pin']);
			$pin = mysqli_real_escape_string($this->db->link, $pin);
			...
			} //password not empty end
			//password empty start
			elseif($npassword=="" && $cnpassword==""){
				if(!empty($file_name)){
					if($file_size>1048567){
						return "<div class='alert alert-danger'><b>OOPS! </b> File's size should be less than 1 MB.</div>";
						...
						$select="SELECT * from tbl_admin where username='$adminUsername'";
		 				$getData=$this->db->select($select);
		 				if($getData){
		 					while($delImg =$getData->fetch_assoc()){
					 			$img=$delImg['img'];
		...
		public function agentEdit($data, $agentEditId){
		  $adminUserame=Session::get('adminUsername');
	      date_default_timezone_set("Asia/Dhaka");
	      $updatedAt= date("Y-m-d H:i:s");
	      //Table and Billing Data
		...
		public function delLoginData($loginDataDeleteId){
		  $loginDataDeleteId=$this->fm->validation($loginDataDeleteId);
		  $loginDataDeleteId= mysqli_real_escape_string($this->db->link, $loginDataDeleteId); 
		  $query="DELETE from tbl_loginhistory WHERE id='$loginDataDeleteId'";
		  $deleteLogin=$this->db->delete($query);
		  if($deleteLogin){
		   return "<div class='alert alert-success'><b>Successfull! </b> Login History Deleted successfully.</div>";
		  }
		  else {
		     return "<div class='alert alert-danger'><b>OOPS! </b> Login History not Deleted.</div>";
		  }
		}
