...
<?php
     $user=new User();
      $ship=new Ship();
    if(!isset($_GET['agentEditId']) || $_GET['agentEditId']==NULL){
      //header("Location: catlist.php");
      echo "<script>window.location='agent.php'</script>";
    }else{
      $agentEditId= preg_replace('/[^-a-zA-Z0-9_]/','',$_GET['agentEditId']);
    }
    if($_SERVER['REQUEST_METHOD']=='POST'){
      $agentEdit=$user->agentEdit($_POST, $agentEditId);
    }
?>
    <!-- **********************************************************************************************************************************************************
        MAIN CONTENT
    ...
    <section id="main-content">
      <section style="min-height:550px;" class="wrapper">
        <?php
          $agentDetails=$user->agentInfoForEdit($agentEditId);
          if($agentDetails){
            while($result=$agentDetails->fetch_assoc()){ ?>
            <h3 style="color: #4ecdc4;"><i class="fa fa-user"></i> <?php echo "Agent- "."<b style='color: #00a99d;'>".$result['name']."</b>";?></h3>
          ...
          <div class="col-lg-12">
            <div style="border-radius: 3px; box-shadow: 0px 0px 8px 3px #aab2bc;" class="form-panel">
              ...
              <h4 style=" padding:10px; color: #4ecdc4;
               background-color: #2f323a; border-radius: 5px;"><i class="fa fa-pencil"></i> Agent Information</h4></br>
              ...
              <div class="form-group">
               ...
               <div style="border-bottom: 0px;" class="form-group">
                  ...
                  <div class="col-md-3 mb-3">
                    <label for="validationServer02">Edited At</label>
                    <input name="editedAt" readonly type="text" class="form-control" id="validationServer04" placeholder="Edited At" value="<?php echo $result['editedAt']?>">
                  </div>
                </div>
                <div class="form-group">
                  <label class="col-lg-2 control-label"></label>
                  ...
                  <div class="col-md-3 mb-3">
                    <label for="validationServer02">Edited By</label>
                    <input name="editedBy" readonly type="text" class="form-control" value="<?php echo $result['editedBy']; ?>">
                  </div>
                </div>
